...
- **L0 — fields → MTxID (OPTIONAL secondary commitment).** Per [P1]: a Merkle tree over the transaction's fields, allowing one field to be shown with an `O(log f)` path **without** the rest of the transaction.
  > **Correction (07_VERIFICATION_FABRIC.md §5).** The BSV block commits to the **consensus TXID** = `reverse(double-SHA256(serialized tx))`, *not* to a Merkle-root-over-fields. So the **inclusion leaf is the consensus TXID**, and the inclusion path is `TXID → subtree → block` (`commitment.VerifyToBlockRoot` with `leaf = txid, l0 = nil`). The L0 field tree (MTxID) is therefore **not on the inclusion path**; it is an *optional secondary* commitment (the secondary identifier of [P1], with its own availability caveat) used for selective field disclosure. Field-level non-disclosure (§4.2) applies to that secondary structure, not to block inclusion; a verifier needing inclusion uses the full TXID. The core inclusion hierarchy is thus **L1–L3** (with L0 optional), and this is what the reference implementation and the verification fabric (§6) verify.
...
6. **Implementation is a reference, not a production benchmark.** A complete zero-dependency Go implementation now exists and its test suite reproduces §6's table and asserts §7's rejections. But the performance figures are arithmetic plus single-machine measurements (per-core hash rate, edge verify rate) used to *derive* capacity — they are **not** a deployed-network throughput benchmark, and §6 R5's core-count is an extrapolation. A production deployment must (a) wire the read-only adapters to a pinned Teranode revision (`01_ARCHITECTURE.md §7` dep #2), (b) use the node's audited secp256k1 — the in-repo signer's secret `k·G` step is now constant-time (`crypto/ct.go`: fixed-limb GF(p), complete a=0 formulas, double-and-add-always, validated by a differential oracle), but the mod-n scalar steps remain `math/big`; this is the *same* secp256k1 curve throughout — and (c) perform full script validation at the till. Value conservation (Σ inputs ≥ Σ outputs) is now implemented via the `ValueOracle` (I-BB6, `walletbob.TestValueConservation`); script-level validation remains node-standard and orthogonal to the MF-SPV proof.
...
test. All 74 pass (`go test ./...`, forced, no cache); `go vet` and `gofmt` clean.
